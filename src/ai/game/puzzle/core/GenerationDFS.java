...
public class GenerationDFS
{
    ...
    public GenerationDFS(String target, String data) 
    {
        this.tempResult = "";
        this.target = target;
        this.data = data;
    ...
    /*Tugas Week 4*/
    public int getHeuristic(String data)// Mismatch
    {
     
            int mismatch=0;
            for(int i=0;i<data.length();i+=1){
                if(data.charAt(i)!=this.target.charAt(i)){
                    mismatch+=1;
                }
            }
        return mismatch;}
    ...
    public int getHeuristicManHattan (String data) // Manhattan Distance
    {
        int manDistance = 0;
        for(int i=0;i<data.length();i+=1){
            for (int j=0;j<this.target.length();j+=1){
                if(data.charAt(i)==this.target.charAt(j))
                    manDistance += ((Math.abs(i%3- j%3))+Math.abs(i/3+j/3));
            }
        }
        return manDistance;
    }
    ...
    public void createChildren()
    {
        ...
        System.out.println("Jumlah iterasi hingga menemukan solusi/final state = "+cnt);
    ...
    public void createLeftChild()
    {
        ...
        if(manipulasiString.getLeftSide() != null && !this.isCreated(manipulasiString.getLeftSide()))
        {
            ...
            openList.add(this.index,this.offSpring);
            this.cntChild++;
            this.tempResult = "";
            }
    }
    ...
    {
        System.out.println("Jumlah anak yang hingga iterasi ditemukan = "+this.cntChild);
    }   
}